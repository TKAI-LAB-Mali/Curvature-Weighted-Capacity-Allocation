...
# (saved in gemma_MDL subdir — spans all methods)
# ---------------------------------------------------------------------------
...
def plot_method_overlay(ratio_name, method_data, norm_type, ratio_dir):
    ...
    Top - Shows Absolute before (shared) and each method's after on the same axes.
          Saved in ratio_dir (the gemma_MDL subdir) since it spans all methods.
...
def main():
    ...
    parser.add_argument("--experiment_dirs", type=str, nargs="+", required=True,
                        help="Explicit paths to the gemma_MDL_* result directories to process.")
    ...
    parser.add_argument("--ratio_paths", type=str, nargs="*", default=None,
                        help=(
                            "Explicit paths to the MDL ratio JSON files, one per ratio set, "
                            "in the same alphabetical order as the gemma_MDL_* result "
                            "directories. When supplied, the sparsity ratio profile is "
                            "overlaid on per-experiment norm plots. If omitted, ratio "
    ...
    ratio_names = [d.name.removeprefix("gemma_MDL_") for d in ratio_dirs]
    ...
    if args.ratio_paths:
        for path_str in args.ratio_paths:
            ...
            name_key = clean_name.removeprefix("rho_l_gemma-7b_")
    ...
    for family, experiments in experiment_groups.items():
        ...
        for r_idx, (ratio_dir, ratio_name) in enumerate(zip(ratio_group_dirs, ratio_group_names)):
            ...
            # Method overlay — saved in the gemma_MDL subdir (ratio_dir)
