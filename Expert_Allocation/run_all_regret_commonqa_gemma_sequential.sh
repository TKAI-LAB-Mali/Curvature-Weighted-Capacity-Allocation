...
for data_path in "${data_paths[@]}"; do
  ...
  # Experts from commonQA is used to fine-tune on ScienceQ, MRPC, CoLA, and OpenBookQA. 
  #The commonQA dataset is NOT fine-tuned on itself, but its experts are used for the other 
  # datasets to measure regret drift. 

  

  output_dir="$root_data_path/training/regret_commonQA/gemma_IF_$filename"
  ...
  --number_experts "1,8,6,4,4,8,11,7,6,9,6,4,7,7,6,5,5,5,3,5,6,5,3,5,4,6,7,8" \
  --top_k "1,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2" \
  --train_on_inputs \
  --group_by_length \
  --add_eos_token \
  --wandb_project "gemma_regret_commonQA" \
  --wandb_run_name "gemma_regret_commonQA_$filename" \
  > "$log_file" 2>&1
