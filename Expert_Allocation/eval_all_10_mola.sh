...
mola_weights=(
  # "$base_weights_path/mistral_alpha_10ep_2b_epoch_glue_rte_all"
  "$base_weights_path/mistral_mola_46810_224_glue_mrpc_all"
  "$base_weights_path/mistral_mola_46810_224_glue_cola_all"
  "$base_weights_path/mistral_mola_46810_224_qa_text_scienceq_all"
  "$base_weights_path/mistral_mola_46810_224_qa_commonq_all"
  "$base_weights_path/mistral_mola_46810_224_qa_openbook_all"
  ...
  for (( i=0; i<${#mola_weights[@]}; i++ )); do
    ...
    if [ -d "$checkpoint_dir" ]; then
      ...
      CUDA_VISIBLE_DEVICES=0,1 python evaluation.py \
        ...
        --number_experts "4,4,4,4,4,4,4,4,6,6,6,6,6,6,6,6,8,8,8,8,8,8,8,8,10,10,10,10,10,10,10,10" \
        --top_k "2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2" \
        --save_path "$base_weights_path/results/mistral_mola_46810/$(basename ${mola_weights[i]})_${checkpoint}.json"
    else
