...
mkdir -p "$root_data_path/mistral_alpha_224_2_$filename"

  # Run experiment
    CUDA_VISIBLE_DEVICES=0,1,2,3 python mola_training_gemma.py \
  --base_model "google/gemma-7b"  \  #"Qwen/Qwen2.5-3B"  \     #"mistralai/Mistral-7B-v0.1"
  --data_path "$data_path" \
  --output_dir "$root_data_path/mistral_alpha_224_2_$filename" \
  --batch_size 128 \
  --micro_batch_size 8 \
  --num_epochs 3 \
  --learning_rate 3e-4 \
  ...
  --number_experts "3,5,7,6,7,7,7,6,5,5,4,4,4,4,5,6,11,6,10,9,9,9,9,9,11,10,8,11,8,9,6,4" \
  --top_k "2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2" \
  --train_on_inputs \
  --group_by_length \
  --add_eos_token \
  --wandb_run_name "mistral_alpha_224_2_$filename"
done
