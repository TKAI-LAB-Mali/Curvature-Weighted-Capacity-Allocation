...
def train(
        ...
        # wandb params
        wandb_project: str = "IF Layer Quality",
        wandb_run_name: str = "",
    ...
    def tokenize(prompt, add_eos_token=True):
        result = tokenizer(
            ...
            padding=False,    #was false before
            return_tensors=None,
        )
        if (
    ...
    class PeftTrainer(Trainer):
        def save_model(self, output_dir=None, _internal_call_model=None, **kwargs):
            """
            Override the default save_model to ensure ONLY the adapters 
            are saved using the custom 'hacked' save function.
            """
            if output_dir is None:
                output_dir = self.args.output_dir

            # Ensure we grab the model from the trainer
            # (Handling potential wrapping like DataParallel)
            model_to_save = self.model
            if hasattr(model_to_save, "module"):
                model_to_save = model_to_save.module

            print(f"*** Custom Saving Adapters to {output_dir} ***")
            
            # This calls your custom 'hacked' save method 
            # (the same one that printed 'save*****' and worked for checkpoint-10)
    ...
    trainer = PeftTrainer(
        ...
        args=transformers.TrainingArguments(
            ...
            save_safetensors=False,  #for gemma
            save_total_limit=6,
            load_best_model_at_end=False,  # True if val_set_size > 0 else False,
            # ddp_find_unused_parameters=False if ddp else None,  ########### commented out
    ...
    model.save_pretrained(output_dir) #for gemma
    end_time = time.time()
    ...
    print(f"Training time: {days}d {hours}h {minutes}m {seconds}s")
