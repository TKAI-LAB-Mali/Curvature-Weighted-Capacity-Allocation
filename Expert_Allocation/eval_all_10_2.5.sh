...
mola_weights=(
  "$base_weights_path/mistral_alpha_15ep_2.5b_epoch_glue_rte_all"
  "$base_weights_path/mistral_alpha_15ep_2.5b_epoch_glue_mrpc_all"
  "$base_weights_path/mistral_alpha_10ep_2.5b_epoch_glue_cola_all"
  "$base_weights_path/mistral_alpha_10ep_2.5b_epoch_qa_text_scienceq_all"
  "$base_weights_path/mistral_alpha_10ep_2.5b_epoch_qa_commonq_all"
  "$base_weights_path/mistral_alpha_10ep_2.5b_epoch_qa_openbook_all"
...
for (( i=0; i<${#mola_weights[@]}; i++ )); do
  for checkpoint in "${checkpoints[@]}"; do
    ...
    if [ -d "$checkpoint_dir" ]; then
      ...
      CUDA_VISIBLE_DEVICES=2,3 python evaluation.py \
        --test_dataset "${test_data[i]}" \
        --base_model "mistralai/Mistral-7B-v0.1" \
        --mola_weights "$checkpoint_dir" \
        --batch_size 8 \
        --lora_target_modules "q_proj,v_proj,k_proj,o_proj,gate_proj,down_proj,up_proj" \
        --number_experts "1,3,5,4,5,5,4,4,3,4,3,2,2,3,3,4,9,4,7,7,7,7,7,7,9,7,6,8,6,7,4,3" \
