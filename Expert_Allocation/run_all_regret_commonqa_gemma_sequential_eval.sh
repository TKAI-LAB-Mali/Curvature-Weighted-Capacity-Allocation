...
sub_directory="regret_commonQA"
EVAL_CHECKPOINTS=false # Set to "False" to only evaluate final model in parent directory (if exists)
model="google/gemma-7b" # Change this to your model if needed
...
#   echo "   >>> Evaluating Run: $run_suffix"
      ...
      # 4. FIND CHECKPOINTS
      training_dir="$root_data_path/training/${sub_directory}/gemma_IF_${filename}"
      ...
      if [ "$EVAL_CHECKPOINTS" = true ]; then
        ...
        fi # End Checkpoint Evaluation Block
