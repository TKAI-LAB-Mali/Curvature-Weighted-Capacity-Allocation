...
mkdir -p "$root_data_path/mistral_IF_25%_epoch_$filename"

  # Run experiment
    CUDA_VISIBLE_DEVICES=0,4,5,6,7 python mola_training_mistral.py \
  --base_model "mistralai/Mistral-7B-v0.1"  \  #"mistralai/Mistral-7B-v0.1"
  --data_path "$data_path" \
  --output_dir "$root_data_path/mistral_IF_25%_epoch_$filename" \
  --batch_size 128 \
  --micro_batch_size 8 \
  --num_epochs 10 \
  --learning_rate 3e-4 \
  ...
  --group_by_length \
  --add_eos_token \
  --wandb_run_name "mistral_IF_25%_epoch_$filename"
done
