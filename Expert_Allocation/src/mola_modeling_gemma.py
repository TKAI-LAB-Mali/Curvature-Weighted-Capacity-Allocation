...
from src.cache_utils import Cache, DynamicCache, StaticCache
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def forward(
        ...
        output_router_logits: Optional[bool] = None, ##newmodified
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    past_key_values,
                    output_attentions,
                    output_router_logits,  # newmodified
                    use_cache,
                    cache_position,
                )
            else:
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            ...
            # TODO joao: remove this `else` after `generate` prioritizes `Cache` objects
                ...
                # --- FIX FOR NEW TRANSFORMERS ---  ########################################### modified 
                if hasattr(past_key_values, "get_seq_length"):
                    cache_length = past_length = past_key_values.get_seq_length()
                else:
                    cache_length = past_length = past_key_values[0][0].shape[2]
        # --------------------------------###################################################modified
...
class GemmaForCausalLM_d(GemmaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
