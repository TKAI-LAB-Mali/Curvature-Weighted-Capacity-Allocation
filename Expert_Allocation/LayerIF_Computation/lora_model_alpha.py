...
class LORAEngineGeneration(object):
    ...
    def load_datasets(self, dataset_name):
        ...
        if dataset_name== 'mrpc':
            ...
            self.validation_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "glue_mrpc_validation_IF-short.hf"))

        if dataset_name== 'cola':
            self.train_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "glue_cola_train_IF-short.hf"))
            self.validation_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "glue_cola_validation_IF-short.hf"))
    
        if dataset_name== 'rte':
            self.train_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "glue_rte_train_IF-short.hf"))
            self.validation_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "glue_rte_validation_IF-short.hf"))
    
        if dataset_name== 'commonq':
            self.train_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "qa_commonq_train_IF-short.hf"))
            self.validation_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "qa_commonq_validation_IF-short.hf"))
    
        if dataset_name== 'openbook':
            self.train_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "qa_openbook_train_IF-short.hf"))
            self.validation_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "qa_openbook_validation_IF-short.hf"))
        
        if dataset_name== 'text_science_q_rebuttal':
            # self.train_dataset = Dataset.load_from_disk(os.path.join(dataset_folder, "qa_text_scienceq_train_IF-30.hf"))
    ...
    def compute_gradient(self, tokenized_datasets, collate_fn, layer):    #LESS
        ...
        self.model.eval()
        tr_grad_dict = {}
        for step, batch in enumerate(tqdm(train_dataloader_stochastic, mininterval=30)):
            # if step==10:
            #     break
            self.model.zero_grad() # zeroing out gradient
        ...
        val_grad_dict = {}
        for step, batch in enumerate(tqdm(val_dataloader_stochastic, mininterval=30)):
            # if step==10:
            #     break
            self.model.zero_grad() # zeroing out gradient
