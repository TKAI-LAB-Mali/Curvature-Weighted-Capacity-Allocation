...
def logUtility(budget: int, layer_costs: list, 
               layer_quality: list,
               max_iter: int = 1000):
    cost = 0
    lambda_value = 0.0
    c_l = layer_costs
    e_l = [1.0] * len(layer_quality)
    ...
    if cost <= budget:
        ...
        iteration = 0
        while abs(g_val) > epsilon and iteration < max_iter:
            iteration += 1
