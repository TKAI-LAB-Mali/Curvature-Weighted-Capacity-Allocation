...
def main():
    ...
    for dataset in datasets:
        ...
        q, lengths = mutil.get_IF(influence_path, dataset, file_attachment)
        ...
        os.makedirs('./data_c/', exist_ok=True, mode=0o755)
        file_path = os.path.join('./data_c/', f"rho_l_gemma-7b_{dataset}_mdl_minmax_{minmax}.json")

        # depth_penalty_subdir = './data/depth_penalty/'
        # os.makedirs(depth_penalty_subdir, exist_ok=True, mode=0o755)
        # file_path = os.path.join(depth_penalty_subdir, f"rho_l_gemma-7b_{dataset}_mdl_{penalty_method}_strength_{strength}_p_{p}_minmax_{minmax}[0:3]_0_3.json")
